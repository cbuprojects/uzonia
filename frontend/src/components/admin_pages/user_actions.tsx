...
const TRANSLATIONS = {
  ...
  ru: {
    ...
    successActions: 'Успешных',
    failedActions: 'Неудачных',
    ...
    previous: 'Назад',
    next: 'Вперёд',
    deleteActionTitle: 'Удалить запись',
  ...
  uz_c: {
    ...
    back: 'Орқага',
    totalActions: 'Жами ҳаракатлар',
    successActions: 'Муваффақиятли',
    failedActions: 'Хатолик',
    uniqueUsers: 'Ноёб фойдаланувчилар',
    colIndex: '#',
    colUsername: 'ЛОГИН',
    colFullName: 'ФИО',
    colDepartment: 'БЎЛИМ',
    ...
    previous: 'Олдинги',
    next: 'Кейинги',
    deleteActionTitle: 'Ёзувни ўчириш',
  ...
  uz_l: {
    ...
    successActions: 'Muvaffaqiyatli',
    failedActions: 'Xatolik',
...
const AdminActionsPage = () => {
  ...
  const currentPath = '/user_actions';
  ...
  const fetchData = useCallback(async () => {
    try {
      const res = await apiFetch('/api/get_all_user_actions');
  ...
  // ── Delete modal ──────────────────────────────────────────────────────────
  const [deleteModal, setDeleteModal] = useState<ActionRecord | null>(null);
  const [isDeleting,  setIsDeleting]  = useState(false);

  // ── Detail popover ────────────────────────────────────────────────────────
  const [detailPopover, setDetailPopover] = useState<ActionRecord | null>(null);
  ...
  const handleDeleteAction = async () => {
    ...
    setIsDeleting(true);
    try {
      const body = { unique_job_id: deleteModal.unique_job_id };
      const res = await apiFetch('/api/delete_user_action', { method: 'DELETE', body: JSON.stringify(body) });
      if (!res || !(res as Response).ok) {
        const e = await (res as Response).json();
  ...
  const filteredActions = useMemo(() => {
    let f = [...actions];
    if (fUsername.trim())   f = f.filter(a => a.username.toLowerCase().includes(fUsername.trim().toLowerCase()));
    if (fFirstName.trim())  f = f.filter(a => a.first_name.toLowerCase().includes(fFirstName.trim().toLowerCase()));
    if (fLastName.trim())   f = f.filter(a => a.last_name.toLowerCase().includes(fLastName.trim().toLowerCase()));
    if (fDepartment.trim()) f = f.filter(a => a.department.toLowerCase().includes(fDepartment.trim().toLowerCase()));
    if (fIpAddress.trim())  f = f.filter(a => a.ip_address?.toLowerCase().includes(fIpAddress.trim().toLowerCase()));
    if (fAction.trim())     f = f.filter(a => a.action.toLowerCase().includes(fAction.trim().toLowerCase()));
    if (fStatus)            f = f.filter(a => a.action_status === fStatus);
    return f;
  }, [actions, fUsername, fFirstName, fLastName, fDepartment, fIpAddress, fAction, fStatus]);

  const stats = useMemo(() => ({
    total:       actions.length,
    success:     actions.filter(a => a.action_status === 'success').length,
    failed:      actions.filter(a => a.action_status === 'failed').length,
    uniqueUsers: new Set(actions.map(a => a.user_id)).size,
  }), [actions]);

  const hasActiveFilters = fUsername || fFirstName || fLastName || fDepartment || fIpAddress || fAction || fStatus;
  const totalPages = Math.ceil(filteredActions.length / itemsPerPage);
  const paginatedActions = useMemo(() =>
    filteredActions.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage),
    [filteredActions, currentPage]
  );

  useEffect(() => { setCurrentPage(1); }, [fUsername, fFirstName, fLastName, fDepartment, fIpAddress, fAction, fStatus]);

  const handleClearFilters = useCallback(() => {
    setFUsername(''); setFFirstName(''); setFLastName('');
    setFDepartment(''); setFIpAddress(''); setFAction(''); setFStatus('');
  }, []);
  ...
  const StatusBadge = ({ status }: { status: ActionStatus }) => {
    const cfg: Record<ActionStatus, { bg: string; color: string; icon: string; label: string }> = {
      success: { bg: '#d1fae5', color: '#065f46', icon: 'check_circle', label: t.success },
      failed:  { bg: '#fee2e2', color: '#991b1b', icon: 'cancel',       label: t.failed  },
    };
  ...
  // ── Navigation button component (same as CalculationsPage) ─────────────────
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
            <span className="material-symbols-outlined" style={{ fontSize: '28px', color: '#0a3b5c' }}>timeline</span>
            <div>
      ...
      <main style={{
        ...
        <div style={{ width: '100%', maxWidth: '1600px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : 'repeat(4, 1fr)', gap: '16px', marginBottom: '24px' }}>
            {[
              { label: t.totalActions,   value: stats.total,       color: '#0a3b5c', iconBg: '#e2e8f0', icon: 'timeline'    },
              { label: t.successActions, value: stats.success,     color: '#065f46', iconBg: '#d1fae5', icon: 'check_circle' },
              { label: t.failedActions,  value: stats.failed,      color: '#991b1b', iconBg: '#fee2e2', icon: 'cancel'       },
              { label: t.uniqueUsers,    value: stats.uniqueUsers, color: '#1d4ed8', iconBg: '#dbeafe', icon: 'group'        },
            ].map(s => (
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'center' }}>
              ...
              {[
                ...
                { icon: 'router',     val: fIpAddress,  set: setFIpAddress,  ph: t.phIpAddress,  flex: '1 1 120px' },
                { icon: 'bolt',       val: fAction,     set: setFAction,     ph: t.phAction,     flex: '1 1 130px' },
              ...
              <select value={fStatus} onChange={e => setFStatus(e.target.value)}
                style={{ ...inputStyle, flex: '1 1 130px', minWidth: '120px', cursor: 'pointer' }}>
            ...
            {hasActiveFilters && (
              <div style={{ marginTop: '10px', fontSize: '12px', color: '#64748b', display: 'flex', alignItems: 'center', gap: '8px', padding: '6px 10px', background: '#f1f5f9', borderRadius: '8px', flexWrap: 'wrap' }}>
                ...
                {t.results(filteredActions.length)}
              </div>
            )}
          </div>
          ...
          <div style={{ background: 'white', borderRadius: '20px', boxShadow: '0 8px 30px rgba(0,40,70,0.08)', overflow: 'hidden', border: '1px solid #e2e8f0' }}>
            {isLoading ? (
              ...
              <div style={{ padding: '60px', textAlign: 'center', color: '#64748b' }}>
                <span className="material-symbols-outlined" style={{ fontSize: '48px', marginBottom: '16px', display: 'block', color: '#94a3b8' }}>timeline</span>
              ...
              <>
                <div style={{ overflowX: 'auto' }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    ...
                    <tbody>
                      {paginatedActions.map((a: ActionRecord, index: number) => {
                        const actualIndex = (currentPage - 1) * itemsPerPage + index + 1;
                        return (
                          <tr key={a.unique_job_id}
                            ...
                            {/* Full name */}
                            <td style={{ padding: '12px 14px' }}>
                              <div style={{ fontSize: '13px', color: '#0f172a', fontWeight: '500', whiteSpace: 'nowrap' }}>
                            ...
                            <td style={{ padding: '12px 14px' }}>
                              <span style={{
                                ...
                                padding: '3px 10px', background: '#e8f0fe', borderRadius: '20px',
                                fontSize: '11px', color: '#0a3b5c', fontWeight: '600', whiteSpace: 'nowrap',
                              }}>
                                <span className="material-symbols-outlined" style={{ fontSize: '12px' }}>domain</span>
                                {a.department || '—'}
                              </span>
                            ...
                            {/* IP */}
                            <td style={{ padding: '12px 14px' }}>
                              <span style={{ fontFamily: 'monospace', fontSize: '12px', color: '#374151', display: 'flex', alignItems: 'center', gap: '4px', whiteSpace: 'nowrap' }}>
                            ...
                            <td style={{ padding: '12px 14px', maxWidth: '220px' }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                                <span style={{ fontSize: '13px', color: '#1e293b', fontWeight: '500' }} title={a.action}>
                                  {truncate(a.action, 35)}
                                </span>
                                {/* Info button to view full details */}
                                <button
                                  onClick={() => setDetailPopover(a)}
                                  title="View details"
                                  style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '2px', display: 'flex', alignItems: 'center', color: '#94a3b8', flexShrink: 0 }}
                                  onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.color = '#0a3b5c'; }}
                            ...
                            <td style={{ padding: '12px 14px' }}>
                              <div style={{ display: 'flex', justifyContent: 'center' }}>
                                <button onClick={() => setDeleteModal(a)} title="Delete record"
                ...
                {filteredActions.length > itemsPerPage && (
                  <div style={{ padding: '20px 24px', borderTop: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#f8fafc', flexWrap: 'wrap', gap: '12px' }}>
                    <div style={{ fontSize: '13px', color: '#64748b' }}>
                      {t.showing((currentPage - 1) * itemsPerPage + 1, Math.min(currentPage * itemsPerPage, filteredActions.length), filteredActions.length)}
                    </div>
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                <h2 style={{ margin: 0, fontSize: '18px', fontWeight: '600', color: '#0a3b5c', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <span className="material-symbols-outlined" style={{ fontSize: '22px', color: '#0369a1' }}>info</span>
                  Action Details
                </h2>
                <button onClick={() => setDetailPopover(null)} style={{ border: 'none', background: '#f1f5f9', cursor: 'pointer', color: '#64748b', width: '36px', height: '36px', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px' }}>×</button>
              </div>

              {/* User info */}
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '14px 16px', background: '#f0f9ff', borderRadius: '10px', marginBottom: '20px', border: '1px solid #bae6fd' }}>
                <div style={{ width: '44px', height: '44px', borderRadius: '50%', background: 'linear-gradient(135deg,#0a3b5c,#1a5080)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: '15px', fontWeight: '700', flexShrink: 0 }}>
                  {(detailPopover.first_name?.[0] ?? '').toUpperCase()}{(detailPopover.last_name?.[0] ?? '').toUpperCase()}
                </div>
                <div>
                  <div style={{ fontWeight: '600', color: '#0a3b5c', fontSize: '15px' }}>{detailPopover.first_name} {detailPopover.last_name}</div>
                  <div style={{ color: '#64748b', fontSize: '12px', display: 'flex', alignItems: 'center', gap: '8px', marginTop: '2px', flexWrap: 'wrap' }}>
                    <span>@{detailPopover.username}</span>
                    {detailPopover.department && (
                      <>
                        <span style={{ color: '#cbd5e1' }}>•</span>
                        <span>{detailPopover.department}</span>
                      </>
                    )}
                  </div>
                </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {[
                  { icon: 'bolt',         label: 'Action',      value: detailPopover.action,          mono: false, wrap: true  },
                  { icon: 'circle',       label: 'Status',      value: null,                          mono: false, wrap: false, badge: detailPopover.action_status },
                  { icon: 'router',       label: 'IP Address',  value: detailPopover.ip_address || '—', mono: true, wrap: false },
                  { icon: 'schedule',     label: 'Timestamp',   value: formatDate(detailPopover.created_at), mono: false, wrap: false },
                  { icon: 'key',          label: t.jobId,       value: detailPopover.unique_job_id,   mono: true, wrap: true  },
                  { icon: 'manage_history', label: t.sessionId, value: detailPopover.session_id,      mono: true, wrap: true  },
                ].map(row => (
                  <div key={row.label} style={{ display: 'flex', gap: '12px', padding: '12px 14px', background: '#f8fafc', borderRadius: '10px', border: '1px solid #e2e8f0', alignItems: row.wrap ? 'flex-start' : 'center' }}>
                    <span className="material-symbols-outlined" style={{ fontSize: '18px', color: '#64748b', flexShrink: 0, marginTop: row.wrap ? '2px' : '0' }}>{row.icon}</span>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: '11px', color: '#94a3b8', fontWeight: '600', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '4px' }}>{row.label}</div>
                      {row.badge ? (
                        <StatusBadge status={row.badge as ActionStatus} />
                      ) : (
                        <div style={{ fontSize: '13px', color: '#0f172a', fontFamily: row.mono ? 'monospace' : 'inherit', wordBreak: row.wrap ? 'break-all' : 'normal' }}>
                          {row.value}
                        </div>
                      )}
                    </div>
              ...
              <div style={{ marginTop: '20px', display: 'flex', justifyContent: 'flex-end' }}>
                <button onClick={() => setDetailPopover(null)} style={{ padding: '10px 24px', background: '#f1f5f9', border: 'none', borderRadius: '10px', fontSize: '14px', fontWeight: '600', color: '#475569', cursor: 'pointer' }}>
                  Close
                </button>
              </div>
            </div>
          ...
          <div onClick={e => e.stopPropagation()} style={{ maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{
              ...
              <div style={{ marginBottom: '28px', padding: '20px', background: '#fef2f2', borderRadius: '12px', border: '1px solid #fee2e2' }}>
                ...
                <div style={{ paddingLeft: '24px', fontSize: '13px', color: '#4b5563', lineHeight: '2.2' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: '6px', marginTop: '4px' }}>
                    <span className="material-symbols-outlined" style={{ fontSize: '14px', color: '#94a3b8', marginTop: '1px' }}>bolt</span>
                    <span>Action: <strong style={{ color: '#1e293b' }}>{truncate(deleteModal.action, 60)}</strong></span>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginTop: '4px' }}>
                    <span className="material-symbols-outlined" style={{ fontSize: '14px', color: '#94a3b8' }}>circle</span>
                    Status: <StatusBadge status={deleteModal.action_status} />
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginTop: '4px' }}>
                    <span className="material-symbols-outlined" style={{ fontSize: '14px', color: '#94a3b8' }}>schedule</span>
                    Time: <strong>{formatDate(deleteModal.created_at)}</strong>
                  </div>
                </div>
              ...
              <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end' }}>
                ...
                <button onClick={handleDeleteAction} disabled={isDeleting}
                  style={{
