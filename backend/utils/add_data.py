...
async def add_new_uzonia_data_to_the_db() -> bool:
    file_path = 'data/excels/uzonia_data_history_2.xlsx'
    ...
    uzonia_data['7-day UZONIA'] = uzonia_data['7-day UZONIA'].astype(float)
    uzonia_data['30-day UZONIA'] = uzonia_data['30-day UZONIA'].astype(float)
    uzonia_data['90-day UZONIA'] = uzonia_data['90-day UZONIA'].astype(float)
    uzonia_data['180-day UZONIA'] = uzonia_data['180-day UZONIA'].astype(float)
    uzonia_data['UZONIA index'] = uzonia_data['UZONIA index'].astype(float)
    ...
    for index, row in uzonia_data.iterrows():
        ...
        next_date = uzonia_data.at[next_index, 'Date']
        ...
        day_7_uzonia = row['7-day UZONIA'] if pd.notnull(row['7-day UZONIA']) else None
        day_30_uzonia = row['30-day UZONIA'] if pd.notnull(row['30-day UZONIA']) else None
        day_90_uzonia = row['90-day UZONIA'] if pd.notnull(row['90-day UZONIA']) else None
        day_180_uzonia = row['180-day UZONIA'] if pd.notnull(row['180-day UZONIA']) else None
        uzonia_index = row['UZONIA index'] if pd.notnull(row['UZONIA index']) else None

        result = await add_new_uzonia_data(unique_job_id=unique_job_id, file_id=file_id,
